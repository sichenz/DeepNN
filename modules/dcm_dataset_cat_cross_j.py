...
def build_dcm_dataset(
    ...
    C = data_j.c.nunique()
    cx = data_j.loc[data_j["j"] == jx, "c"].values[0]
    ...
    while not sampler_empty:
        ...
        _purchase_history_category = tmp[2].dot(aggregation_j_c)[:, :, cx]
        for i, w in enumerate(bc_average_window_sizes):
            bc_average_features_category[:, i] = _purchase_history_category[
                :, -w:
            ].mean(axis=1)
        ...
        x = np.hstack(
            [
                ...
                bc_average_features,  # B x W (product)
