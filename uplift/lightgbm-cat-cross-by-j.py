...
import modules.dcm_dataset_cat_cross


def main(x, path_data, **kwargs):

    logger.info("Discount simulation `lightgbm-cat-cross by j`")
    ...
    label = "LightGBM_Cat_Cross_ByJ"
    label2 = "total_prob_lightgbm_cat_cross_by_j"
    logger.info(f"label={label}")

    emb = "w2v"  # hardcode for now
    if emb == "w2v":
    ...
    _, X_test, index_it_test = modules.dcm_dataset_cat_cross.build_dcm_dataset(
    ...
    W = len(config["training"]["avg_windows"])
    assert X_test.shape[1] == 3 + J + W + C + W
    ...
    def predict_jx(
        jn, X_test, discount, prediction, models, baseline_pf, streamer_test, W
    ):

        cn = data_j[data_j.j.values == jn].c.values[0]
        ...
        x_input[:, (3 + J + W) : (3 + J + W + C)] = 0
        x_input[:, (3 + J + W) + cn] = discount  # category-level cross + own
...
if __name__ == "__main__":
    ...
    args = modules.args.global_args("Discount simulation `lightgbm-cat-cross by j`")
