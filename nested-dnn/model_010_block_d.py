...
class Model(ModelBase):
    """
    model 005 + block-diagonal discount bottleneck
    ...
    def __init__(self, J, T, K, L, epsilon, pretrained):
        ...
        self.w_conv_j_diag_d = nn.Parameter(
            torch.FloatTensor(J).uniform_(-0.025, 0.025)
        )
        self.w_conv_j_offdiag_d = nn.Parameter(
    ...
    def forward(self, in_pf, in_np, in_bc):
        ...
        # discount cross
        w_conv_manual_diag_d = self.w_conv_j_diag_d * self.eye_j
        w_conv_manual_offdiag_d = (
            self.eye_block_c
            * self.w_conv_j_offdiag_d.repeat((10, 1)).transpose_(0, 1).flatten()
        )
        w_conv_manual_d = w_conv_manual_diag_d + w_conv_manual_offdiag_d
        cross_price_out = torch.mm(in_np, w_conv_manual_d)
