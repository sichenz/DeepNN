...
import modules.dcm_dataset_cat_cross_j


def main(x, emb, path_data, **kwargs):

    logger.info("Baseline `lightgbm-cat-cross by j`")
    ...
    n_cores = config["n_cores"]["lightgbm-cat"]
    ...
    def train_jx(
        ...
        y_train, X_train, index_it = modules.dcm_dataset_cat_cross_j.build_dcm_dataset(
        ...
        assert (
            X_train.shape[1] == 3 + J + 2 * len(config["training"]["avg_windows"]) + C
        )
        ...
        (
            ...
            X_test,
            index_it_test,
        ) = modules.dcm_dataset_cat_cross_j.build_dcm_dataset(
